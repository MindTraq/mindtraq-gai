...
export const getTokens = async () => {
    return JSON.parse(localStorage.getItem('mindtraq.auth.tokens'));
}

// Saves tokens to persistent storage
export const setTokens = async (value) => {
    if(value === null) localStorage.removeItem('mindtraq.auth.tokens');
    else localStorage.setItem('mindtraq.auth.tokens', JSON.stringify(value));
}
