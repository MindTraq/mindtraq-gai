...
const analyzeUserInputModel = genAI.getGenerativeModel({
    model: "gemini-1.5-pro",
    systemInstruction: Prompts.ANALYZE_USER_INPUT,
    generationConfig: GAI_GENERATION_CONFIG
});
const analyzeDayModel = genAI.getGenerativeModel({
    model: "gemini-1.5-pro",
    systemInstruction: Prompts.ANALYZE_DAY,
    generationConfig: GAI_GENERATION_CONFIG
});
    ...
    llm: {
        ...
        */
        query: async (type, input, schema = null, retry = 0) => {
            ...
            try {
                let result;
                if (type === 'ANALYZE_USER_INPUT') result = await analyzeUserInputModel.generateContent(input);
                else if (type === 'ANALYZE_DAY') result = await analyzeDayModel.generateContent(input);
        ...
        */
        analyzeUserInput: async (input) => {
            ...
            return API.llm.query('ANALYZE_USER_INPUT', input, schema);
        },
        ...
        */
        analyzeDay: async (input) => {
            ...
            return API.llm.query('ANALYZE_DAY', input, schema);
        }
